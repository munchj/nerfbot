  ...
  ws.on('message', function incoming(message) {
    ...
    try {
      ...
      if (messageObject.type == c.MSG_MOVE_TURRET) {
        let directionX = messageObject.speedX < 0 ? c.ARDUINO.FORWARD : c.ARDUINO.BACKWARDS;
        let directionY = messageObject.speedY >= 0 ? c.ARDUINO.BACKWARDS : c.ARDUINO.FORWARD;
        arduinoWrapper.rotate(Math.abs(messageObject.speedX), directionX, Math.abs(messageObject.speedY), directionY);
      }
